...
# Cola de prioridad: (prioridad, tiempo_evaluacion, contador, objeto_paciente)
cola_pacientes = []
contador_global = 0
...
class Paciente:
    def __init__(self, env, nombre):
        ...
        env.process(self.proceso_paciente())
...
def gestor_atencion(env, medicos, salas):
    while True:
        if cola_pacientes and medicos.count < medicos.capacity and salas.count < salas.capacity:
            ...
            with medicos.request(priority=paciente.prioridad) as req_medico, salas.request(priority=paciente.prioridad) as req_sala:
...
salas = simpy.PriorityResource(env, capacity=NUM_SALAS)
